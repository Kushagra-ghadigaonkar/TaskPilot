...
#   access_entries = local.access_entries   # if i want to give access to some other user (for now its not required to me)
  ...
  enabled_log_types                      = []              # To enable log saving and authentication for control plane
  create_cloudwatch_log_group = false
  # cloudwatch_log_group_retention_in_days = 7                                       # Save logs of last 7 days


  vpc_id                   = module.vpc.vpc_id
  subnet_ids               = module.vpc.public_subnets  # private subnet allow outbound traffic which enhances security thats why it is used
